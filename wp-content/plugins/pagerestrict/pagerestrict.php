...
function pr_get_page_content() {
	...
	if ( pr_get_opt ( 'loginform' ) == true ) :
		...
		' . $errors . '
			<p>
				<label for="log">'.apply_filters( 'pr_username_label', 'Username' ) . '</label><input type="text" name="log" id="log" value="' . esc_html ( stripslashes ( $user_login ) , 1 ) . '" size="22" /> <br />
				<label for="pwd">' . apply_filters( 'pr_password_label' , 'Password' ) . '</label><input type="password" name="pwd" id="pwd" size="22" /> <br />
		...
		';*/
		$pr_page_content .= do_shortcode('[RM_Login]');
		$pr_page_content .= do_shortcode('[TheChamp-Login]');
		global $post;
