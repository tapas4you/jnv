...
class RM_DBManager
{
    ...
    public static function get_submissions_for_user($user_email, $limit = 0, $offset = 0, $sort_by = '', $descending = false)
    {
        ...
        if ($descending === false)
        {
            $results = $wpdb->get_results("SELECT * FROM `$table_name` WHERE `user_email` = '$user_email' AND `child_id` = 0 ORDER BY `$sort_by` $limit_query OFFSET $offset");
        } else
        {
            $results = $wpdb->get_results("SELECT * FROM `$table_name` WHERE `user_email` = '$user_email' AND `child_id` = 0 ORDER BY `$sort_by` DESC $limit_query OFFSET $offset");
        }
